...
import { setCustomerSearch, toggleCustomerStatus, fetchCustomers } from '../../../features/adminCustomersSlice';
import { Search, Ban, CheckCircle, Mail, Phone, History } from 'lucide-react';

const CustomersPage = () => {
    const dispatch = useDispatch();
    const { filteredCustomers, searchQuery } = useSelector((state) => state.adminCustomers);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredCustomers.length > 0 ? (
                    filteredCustomers.map((customer) => (
                        <div key={customer.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col transition-all hover:shadow-md">
                            ...
                            <div className="border-t border-gray-100 my-4 pt-4 grid grid-cols-2 gap-4">
                                ...
                                <div className="text-center border-l border-gray-100">
                                    ...
                                    <p className="text-lg font-bold text-brand-mahogany">₹{customer.totalSpent}</p>
                                </div>
                            </div>

                            <div className="flex gap-2 mt-auto">
                                <button className="flex-1 py-2 border border-gray-200 rounded-lg text-smfont-medium text-gray-600 hover:bg-gray-50 flex items-center justify-center gap-2">
                                    <History size={16} />
                                    History
                                </button>
                                <button
                                    onClick={() => dispatch(toggleCustomerStatus(customer.id))}
