        ...
        const startPolling = () => {
            interval = setInterval(async () => {
                ...
                try {
                    ...
                    if (data.status === 'DELIVERED' || data.status === 'CANCELLED' || data.status === 'Delivered' || data.status === 'Cancelled') {
                        clearInterval(interval);
                    }

                    setOrder(data);
                } catch (err) {
                    // Silently fail on network error retry, or stop if 404? 
                    // User req: "404 order → stop polling". 
                    // But trackOrder throws error msg. We can check err message or status if available.
                    // For now, if it fails, we keep polling unless it's a hard error, but strictly per requirements:
