...
import { setFilterStatus, setSearchQuery, updateOrderStatus, fetchOrders } from './adminOrdersSlice';
                ...
                <div className="hidden md:block overflow-x-auto bg-white rounded-xl border border-gray-100 md:border-0">
                    <table className="w-full text-left">
                        <thead className="bg-gray-50 text-gray-600 font-semibold text-sm">
                            <tr>
                                <th className="md:px-3 md:py-3 lg:px-6 lg:py-4 whitespace-nowrap">Order ID</th>
                                <th className="md:px-3 md:py-3 lg:px-6 lg:py-4 whitespace-nowrap">Customer</th>
                                <th className="md:px-3 md:py-3 lg:px-6 lg:py-4 whitespace-nowrap">Date</th>
                                <th className="md:px-3 md:py-3 lg:px-6 lg:py-4 whitespace-nowrap">Total</th>
                                <th className="md:px-3 md:py-3 lg:px-6 lg:py-4 whitespace-nowrap">Status</th>
                                <th className="md:px-3 md:py-3 lg:px-6 lg:py-4 text-right whitespace-nowrap">Actions</th>
                            </tr>
                        </thead>
                            ...
                            {filteredOrders.length > 0 ? (
                                filteredOrders.map((order) => (
                                    <tr key={order.id} className="hover:bg-gray-50 transition-colors">
                                        <td className="md:px-3 md:py-3 lg:px-6 lg:py-4 font-medium text-gray-900 whitespace-nowrap">{order.orderId}</td>
                                        <td className="md:px-3 md:py-3 lg:px-6 lg:py-4 text-gray-700 whitespace-nowrap">{order.customer?.name || order.customer}</td>
                                        <td className="md:px-3 md:py-3 lg:px-6 lg:py-4 text-gray-500 text-sm whitespace-nowrap">{order.date}</td>
                                        <td className="md:px-3 md:py-3 lg:px-6 lg:py-4 font-semibold text-gray-900 whitespace-nowrap">₹{order.total}</td>
                                        <td className="md:px-3 md:py-3 lg:px-6 lg:py-4 whitespace-nowrap">
                                        ...
                                        <td className="md:px-3 md:py-3 lg:px-6 lg:py-4 text-right whitespace-nowrap flex items-center justify-end gap-2">
                                            <select
                                                onClick={(e) => e.stopPropagation()}
                                                value={order.status}
                                                onChange={(e) => handleStatusUpdate(order.id, e.target.value)}
                                                className={`text-xs font-medium border-gray-200 rounded-lg py-1 pl-2 pr-6 bg-white focus:ring-2 focus:ring-brand-mahogany outline-none cursor-pointer border shadow-sm h-8
                                                    ${order.status === 'DELIVERED' ? 'text-green-700 bg-green-50' :
                                                        order.status === 'PLACED' ? 'text-yellow-700 bg-yellow-50' :
                                                            order.status === 'PREPARING' ? 'text-blue-700 bg-blue-50' :
                                                                order.status === 'OUT_FOR_DELIVERY' ? 'text-purple-700 bg-purple-50' :
                                            ...
                                            <button
                                                ...
                                                className="text-gray-400 hover:text-brand-mahogany transition-colors p-1"
                                                title="View Details"
                                            >
                                                <Eye size={20} />
                                            </button>
                            ...
                            <div key={order.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col gap-3">
                                ...
                                <div className="flex items-center justify-between pt-1">
                                    ...
                                    <select
                                        ...
                                        value={order.status}
                                        onChange={(e) => handleStatusUpdate(order.id, e.target.value)}
                                        className={`text-xs font-medium border-gray-200 rounded-lg py-1.5 pl-2 pr-6 bg-white focus:ring-2 focus:ring-brand-mahogany outline-none cursor-pointer border shadow-sm
                                            ${order.status === 'DELIVERED' ? 'text-green-700 bg-green-50' :
                                                order.status === 'PLACED' ? 'text-yellow-700 bg-yellow-50' :
                                                    order.status === 'PREPARING' ? 'text-blue-700 bg-blue-50' :
                                                        order.status === 'OUT_FOR_DELIVERY' ? 'text-purple-700 bg-purple-50' :
                                    ...
                                    <button
                                        onClick={() => setSelectedOrder(order)}
                                        className="flex items-center gap-2 px-3 py-1.5 bg-gray-50 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-100 active:scale-95 transition-all border border-gray-200"
                                    >
                                        <Eye size={16} />
                                        View
                                    </button>
